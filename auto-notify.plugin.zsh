1export AUTO_NOTIFY_VERSION="0.8.0"
...
function _auto_notify_message() {
    ...
    title="[ "$hostname" ] "$command" has finished ["$elapsed"s]"
    body="Completed in "$elapsed"sec. with exit code "$exit_code" @ "$hostname""
    ...
    if [[ "$platform" == "Linux" ]]; then
        ...
        printf "Please post an issue on gitub.com/D3STY/zsh-auto-notify/issues/\n"
    fi
}
